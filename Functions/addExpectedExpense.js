...
export const handler = async function (event, context) {
  ...
  const exists = period.expectedExpenses.find(
    (ee) => ee.id == payload.expectedExpense.id
  );

  let newExpectedExpenses = [];

  if (exists) {
    newExpectedExpenses = period.expectedExpenses.map((ee) =>
      ee.id === payload.expectedExpense.id ? payload.expectedExpense : ee
    );
  } else {
    newExpectedExpenses = [...period.expectedExpenses, payload.expectedExpense];
  }
